    ...
    return (
        <div className={twMerge("prose prose-slate max-w-none prose-headings:font-bold prose-a:text-primary-600 hover:prose-a:text-primary-700", className)}>
            <ReactMarkdown
                ...
                components={{
                    p: ({ node, ...props }: any) => <p {...props}>{props.children}</p>,
                    span: ({ node, ...props }: any) => <span {...props}>{props.children}</span>,
                    div: ({ node, ...props }: any) => <div {...props}>{props.children}</div>,
                    table: ({ node, ...props }: any) => (
                        <div className="overflow-x-auto my-4 border border-slate-200 rounded-lg">
                            <table className="w-full text-left text-sm" {...props} />
                        </div>
                    ),
                    thead: ({ node, ...props }: any) => <thead className="bg-slate-50 border-b border-slate-200" {...props} />,
                    th: ({ node, ...props }: any) => <th className="px-4 py-3 font-semibold text-slate-700" {...props} />,
                    td: ({ node, ...props }: any) => <td className="px-4 py-3 border-b border-slate-100 last:border-0" {...props} />,
                    code: ({ node, className, children, ...props }: any) => {
                        const match = /language-(\w+)/.exec(className || '');
                        const isBlock = !!match;

                        // Strip node and other non-standard props
                        ...
                        return (
                            <code className="bg-slate-100 text-slate-800 px-1.5 py-0.5 rounded text-sm font-mono" {...cleanProps}>
                                {children}
                            </code>
                        );
                    },
