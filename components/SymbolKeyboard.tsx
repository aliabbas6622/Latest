...
import { Sigma, FlaskConical, Atom, X, ChevronRight } from 'lucide-react';
    ...
    return (
        ...
        >
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {group.symbols.map((symbol, sIdx) => (
                                <button
                                    ...
                                    className="bg-white border border-slate-100/80 h-14 rounded-2xl flex items-center justify-center hover:border-primary-400/50 hover:bg-primary-50/30 hover:shadow-sm transition-all group active:scale-90 px-2 overflow-hidden"
                                    title={symbol}
                                >
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-center gap-3">
                <div className="flex items-center gap-2 px-4 py-2 bg-primary-600/5 rounded-full border border-primary-100">
                    <ChevronRight size={14} className="text-primary-600" />
                    <p className="text-[10px] font-black text-primary-700 uppercase tracking-wider">Click symbol to insert at cursor</p>
                </div>
            </div>
        </motion.div>
    );
