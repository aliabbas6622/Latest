...
const SuperAdminMaterials = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm("Delete this study material?")) {
      try {
        await db.deleteMaterial(id, user.role as any);
        setRefresh(p => p + 1);
      } catch (e: any) {
        alert(e.message);
      }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    try {
      await db.addMaterial(newMat, user.role as any);
      setIsAdding(false);
      setRefresh(p => p + 1);
      setNewMat({ ...newMat, title: '', content: '', summary: '', topicId: 'topic-' + Date.now() });
    } catch (e: any) {
      alert(e.message);
    }
  };
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <input
                ...
                required
                value={newMat.topic}
                onChange={e => setNewMat({ ...newMat, topic: e.target.value, topicId: e.target.value.toLowerCase().replace(/\s+/g, '-') })}
              />
            </div>
            <input
